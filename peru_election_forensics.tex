...
\author{Carlos C\'esar Ch\'avez Padilla%
  \thanks{Center for the Economics of Human Development,
          ...
          mesa-level data publicly available.
          All analysis was conducted in Python; replication code and data
...
of valid votes in southern highland provinces while Fujimori dominated coastal
...
substantial methodological debate: \citet{escobari2024} argue that the OAS
claimed fraud on the basis of faulty statistical reasoning, while
\citet{idrobo2022} show that late-counted rural votes mechanically shifted
results toward Evo Morales, consistent with normal differential reporting
speeds rather than manipulation. Peru 2021 differs from Bolivia 2019 in a
...
find no evidence supporting them. Similar dynamics unfolded in Kenya's 2017
election and, to varying degrees, in multiple Latin American contests over the
last decade \citep{lehoucq2003, hyde2007}. In each case, the gap between
forensic evidence and public perception is large. Statistical analysis alone
cannot close that gap, but it provides the evidentiary baseline against which
other claims must be weighed.
...
system, which \citet{fujiwara2015} and others have documented as one of the
...
The sum of all 86,488 rows in the ONPE file gives a Castillo--Fujimori margin
of 44,240 votes, within 23 votes of the official JNE-certified margin of
44,263. The trivial discrepancy comes from rounding in the small number of
dropped rows (ANULADA and EN PROCESO). The data are internally consistent.

\medskip
\noindent [Table~\ref{tab:summary} about here]

\medskip
\noindent [Table~\ref{tab:flags} about here]
...
\medskip
\noindent [Figure~\ref{fig:map_castillo} about here]
...
\medskip
\noindent [Figure~\ref{fig:dept_bars} about here]
...
\medskip
\noindent [Table~\ref{tab:contested} about here]
...
\medskip
\noindent [Figure~\ref{fig:map_contested} about here]
...
\medskip
\noindent [Figure~\ref{fig:klimek} about here]
...
\medskip
\noindent [Table~\ref{tab:digits} about here]
...
\medskip
\noindent [Figure~\ref{fig:lastdigit} about here]
...
\medskip
\noindent [Table~\ref{tab:regression} about here]
...
\medskip
\noindent [Figure~\ref{fig:primera_segunda} about here]
...
\medskip
\noindent [Figure~\ref{fig:turnout} about here]
...
\medskip
\noindent [Figure~\ref{fig:map_nullvotes} about here]
...
observed pattern points instead to protest voting by urban Lim\~enhos
...
she would be convicted and imprisoned (as ultimately occurred in 2023).
...
had occurred \citep{chumacero2025}.
...
Kenya in 2017 \citep{gallego2023}. Rapid communication of forensic evidence
...
The anomaly flag counts in Table~\ref{tab:flags} reveal no systematic
indicators of fraud. The 319 mesas with high null rates deserve further
inspection: as Figure~\ref{fig:map_nullvotes} shows, they are concentrated
in coastal departments rather than in Castillo's highland strongholds,
consistent with protest voting rather than targeted invalidation of
pro-Castillo ballots.
...
The data in Table~\ref{tab:contested} show that contested mesas were drawn
predominantly from areas where Fujimori was already the stronger candidate.
The JNE's resolution of these challenges brought them into the final count
with a Fujimori margin of 39,250---consistent with their geographic origin,
not with a pro-Castillo manipulation.
...
Table~\ref{tab:digits} shows uniformity across all four subgroups.
The fact that the contested and uncontested groups produce nearly identical
test results---despite different geographic compositions---provides direct
evidence against the hypothesis that vote totals in challenged actas were
altered.
...
The regression coefficients in Table~\ref{tab:regression} confirm that
geography explains most of the second-round variation (R$^2 = 0.71$)
and that contested-mesa concentration adds nothing to the explanation.
The small negative point estimate on share contested ($\hat\gamma = -0.113$)
reflects the fact that contested mesas cluster in Fujimori territory.
...
The following map shows the geographic distribution of Castillo's valid-vote
share across Peru's 195 provinces. The north-to-south highland corridor and the
Amazonian east-northeast voted overwhelmingly for Castillo; the coast and the
Lima metropolitan area voted for Fujimori by large margins.
...
The spatial pattern in Figure~\ref{fig:map_castillo} is the starting point
for interpreting every subsequent forensic test. High Castillo shares in the
sierra are genuine: they replicate across methods, align with first-round
performance, and are not accompanied by any of the statistical signatures of
manipulation.
...
The Klimek fingerprint plots in Figure~\ref{fig:klimek} test for ballot
stuffing (which would produce a ``finger'' extending toward the high-turnout,
high-share corner) and for vote stealing (which would shift the cloud rightward
without inflating turnout). Contested mesas are highlighted in orange to allow
direct inspection of whether challenged actas occupy a forensically suspicious
...
The two regional clusters---high-Castillo Sierra mesas and high-Fujimori
Costa mesas---reflect geography, not manipulation. The fingerprint method
requires that a fraud cluster appear as an outlying extension of the
main distribution; no such extension is visible in either panel.

\newpage

% ─── Figure 3 ─────────────────────────────────────────────────────────────────

Figure~\ref{fig:lastdigit} displays the empirical digit distributions for all
four subgroups. Under the null hypothesis of genuine counting, each bar should
be near 10\,\%. The key diagnostic is whether bars at 0 and 5 are elevated
relative to other digits, which is the heaping signature documented in
fabricated data \citep{beber2012}.
...
The four panels in Figure~\ref{fig:lastdigit} are visually uniform.
Digits 0 and 5 show no systematic elevation. The contested panels
(bottom row) are noisier due to smaller samples but show no heaping pattern.
Taken together with Table~\ref{tab:digits}, these plots provide strong
visual confirmation of the chi-squared test results.

\newpage

% ─── Figure 4 ─────────────────────────────────────────────────────────────────

The scatter in Figure~\ref{fig:primera_segunda} shows the cross-district
relationship between first- and second-round vote shares for Castillo/Peru Libre.
The relationship is tight, confirming that second-round geography was largely
determined by first-round geography. Districts that over-perform relative
to the fitted line (positive residuals) deserve scrutiny as potential fraud
indicators.
...
Orange dots (districts with at least one contested mesa) show no systematic
deviation above the regression line, consistent with Specification~2 of
Table~\ref{tab:regression}. The districts with the largest over-performance
(top-right outliers) are in Loreto and Amazonas---remote departments where
Peru Libre had low first-round shares but where consolidation around Castillo
in the runoff was particularly sharp.

\newpage

% ─── Figure 5 ─────────────────────────────────────────────────────────────────

Turnout in the segunda vuelta averaged 76.1\,\% domestically, lower than
in the primera vuelta. Figure~\ref{fig:turnout} shows the full turnout
distributions for both rounds, allowing direct comparison of their shapes.
Ballot stuffing would produce an upward shift and possible right-tail
distortion in the segunda vuelta distribution relative to the primera.
...
The segunda vuelta distribution in Figure~\ref{fig:turnout} is shifted
slightly to the left of the primera vuelta distribution---turnout was lower,
not higher, in the closer race. This is the opposite directional prediction
under ballot stuffing. Both distributions are unimodal and symmetric,
with no secondary spike near 100\,\%.

\newpage

% ─── Figure 6 ─────────────────────────────────────────────────────────────────

Figure~\ref{fig:dept_bars} summarizes the department-level results that
underlie the geographic map in Figure~\ref{fig:map_castillo}. Each bar shows
Castillo's valid-vote share, with the national average (50.44\,\%) as a
reference. The sorting is nearly perfect: all highland departments are above
the national mean and all coastal departments are below.
...
The department-level spread shown in Figure~\ref{fig:dept_bars} spans over
50 percentage points between the top and bottom departments.
This degree of geographic sorting makes Peru 2021 an unusually demanding
test case for forensic methods calibrated on less polarized electorates.

\newpage

% ─── Figure 7 ─────────────────────────────────────────────────────────────────

Null vote rates provide a geographic check on one form of fraud allegation:
systematic invalidation of votes to reduce a candidate's total.
Figure~\ref{fig:map_nullvotes} maps the null rate at the province level.
If Castillo votes were being invalidated in highland provinces, the map
should show elevated rates in the same areas as Figure~\ref{fig:map_castillo}.
...
Figure~\ref{fig:map_nullvotes} shows a geographic pattern inverse to
Figure~\ref{fig:map_castillo}: elevated null rates appear in coastal provinces
(Lima, Piura, La Libertad, Lambayeque), not in the southern highlands.
Castillo's strongholds show among the lowest null rates in the country.
This is inconsistent with targeted invalidation of Castillo votes and
consistent with urban protest voting.

\newpage

% ─── Figure 8 ─────────────────────────────────────────────────────────────────

The final figure maps the geographic distribution of contested mesas by province.
If challenged actas were concentrated in areas where Castillo was strong,
that would support the interpretation that Fujimori's legal team targeted
Castillo-favoring polling stations. The map allows direct visual inspection.
...
Figure~\ref{fig:map_contested} shows the highest contestation rates
concentrated in Lima and the northern coast---the heart of Fujimori's
support. The southern highland provinces where Castillo received his largest
vote shares show among the lowest contestation rates nationwide.
This geographic inversion is the clearest single-image summary of why
the fraud allegations are not supported by the data.
